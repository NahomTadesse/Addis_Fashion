...
import classes from './ProductPage.module.css';
import { useCookies } from 'react-cookie';
import { getValidToken, isValidToken } from '../authService';
...
function ProductPageContent() {
  ...
  useEffect(() => {
    ...
    const checkAuth = async () => {
      ...
      setAuthChecked(true);
      // fetchCartItems(); // Uncomment if this function exists
    };
    ...
    if (searchParams) {
      ...
      if (productParam) {
        try {
          ...
          setActiveImage(
            `https://fashion-api.addispages.com/api/v1/file/${decodedProduct.imageUrl.split('_uploads/')[1]}`
          );
          setLoading(false);
        } catch (error) {
          console.error('Error parsing product data:', error);
...
export default function ProductPage() {
  return (
    <Suspense fallback={<Container size="xl" py={40}><Text ta="center">Loading product...</Text></Container>}>
      <ProductPageContent />
    </Suspense>
  );
}